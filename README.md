...
codeman agent inbox --wait 60000                          # read my own mailbox; reading does NOT ack
codeman agent ack                                         # ack what I read (all of it, or `ack <id…>`); --peek to read unmarked
...
codeman agent ls                                          # INBOX = unread posts, WAIT = parked on `inbox --wait` since; footer names a stalled pair
codeman agent restore "$SID"                              # respawn a dead pane with its conversation; refuses a live one
codeman agent restore --last                              # rebuild the most recently DELETED session, resuming its conversation
```

**Restoring a deleted session.** `rm` is the recommended end of a worker's life, so the session most worth bringing back is usually gone, not merely dead. `restore` handles both: a live session with a dead pane goes to the host's `codeman-restore-session`; a session that was **deleted** is rebuilt from the `deleted` entry the server writes to the lifecycle log, resuming its CLI conversation via `POST /api/sessions`. The conversation id comes from `--resume <cli-id>` if given, else the id the server recorded (claude/codex), else per-mode discovery — opencode's own `session list` finds the newest conversation in the working directory (Codeman never learns a fresh opencode `ses_…` id). A remote (SSH) session, or one whose log entry predates this feature (no `workingDir`), is refused with a hint. The same verb exists without a session context as `codeman session restore [id] [--last] [--url <url>]`.

**Lifecycle rules for orchestrators.** (1) A worker you replaced is a worker you delete: after the successor is spawned and has its handoff, `rm` the predecessor — a finished session still holds a tmux pane, a case slot and, for opencode, a running MCP tree. (2) Spawn opencode workers with `--permission allow`: without it every new command pattern stops the worker on an "Allow once / Allow always / Reject" dialog that nobody answers (granting the permission is the operator's call — the flag only carries it). (3) `status` is a UI hint: use `ls --alive` (or `wait --until exit --timeout 1000`) before deciding a worker is done, and `restore` when its pane died (a stray `Ctrl+C` is `app_exit` in opencode). `restore` delegates to the host's `codeman-restore-session` tool, which respawns the pane from the original launch command and re-attaches the CLI's last conversation; `--resume <cli-session-id>` picks an exact one, and `--last` rebuilds the most recently deleted session from its lifecycle record (see below).

**Mailbox vs. `send`.** `send` types into the receiver's composer: right for a prompt, wrong for a message — it starts or interrupts a billed turn, the receiver cannot defer it, and in a fullscreen TUI a stray byte is a dead session. `post` stores instead: the message sits in the receiver's inbox until it runs `codeman agent inbox` (between its own steps, or blocking with `--wait`), and it stays there until `codeman agent ack`. **Reading is not acknowledging:** `inbox` is non-destructive and only marks what it printed as seen, so an order that was read but not acted on remains `pending` — visible in `ls`/`summary` and re-delivered on the next read — instead of vanishing with the read. Ack exactly what you read (no ids = everything this session has read; a message posted after the read survives), and prefer to do it after the work is done or handed off. Inboxes are bounded (200 messages × 16 KB; a full inbox refuses the post so the sender knows), persisted in the data dir (`agent-inbox.json`, so they survive a server restart), dropped with the session, and announced to browsers as the `inbox:message` SSE event. Nothing is written to the pane — a receiver that never polls never sees the message, by design; put "check `codeman agent inbox` at the start of each turn" into the worker's brief. **Reply before you wait.** Two agents that each park on `inbox --wait` for the other's post stall silently — both `idle`, both waiting, nothing on any screen. `ls` makes that visible: `INBOX` is the unread count, `WAIT` says since when a session has been parked on its inbox, and a footer names a waiting session whose parent or child is idle with an empty inbox ("looks like someone owes a post"). The rule that prevents it: after reading a report, post the answer (approval, next task, or an explicit "stop") *before* arming your own wait.

Rules the commands enforce rather than document: they refuse outside a Codeman session and never guess a URL; `send` transmits printable text plus Enter only (a control byte such as `Ctrl+C` is `app_exit` in opencode — ESC exists solely as `interrupt`, which never appends Enter); `rm` refuses an empty id, an unprovable self id and a prefix match in either direction. Ids may be the 8-character prefixes `ls` prints (resolved through the list; an ambiguous prefix refuses). Exit codes: `0` delivered/matched/signal, `1` error, `2` timeout, `3` the worker exited or the wait ended without an answer (`delivered:false`, `ended:true`), `4` refused. `spawn` prints the id alone on stdout (prose goes to stderr), so `SID=$(…)` captures exactly the id. `--json` prints the envelope's `data` for every verb. `post`/`inbox`/`ack` are the mailbox (below), the only trio that never touches a pane. `--until stop` on a mode without hook signals is the server's 400, passed through — the marker path (`--match`) is the answer there, exactly as for the skill.
...
REST over Fastify — **~205 handlers across 22 route modules**, plus an SSE stream and a WebSocket terminal channel. All responses use the `ApiResponse<T>` envelope (`{success, data}` / `{success, error, errorCode}`); `/api/v1/*` is a stable alias. A representative subset:
